    ...
    for (p1 = p2 = worklist; NULL != p1; p1 = p2) {
        ...
        if (p1->counters.last_updated + (config->checkinterval * config->clienttimeout) <= current_time) {
            ...
            if (config->auth_servers != NULL) {
                ...
                case AUTH_MEMBER:
                    if (tmp->fw_connection_state != FW_MARK_MEMBER) {
                        debug(LOG_INFO, "%s - Access has changed to member, refreshing firewall and clearing counters",
                              tmp->ip);
